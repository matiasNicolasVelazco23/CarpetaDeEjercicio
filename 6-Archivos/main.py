...
#leer json, pasar de json a una lista de diccionarios "abrir", accedo a la clave bzrp, 
# que tiene una lista de diccionarios
def parse_json(nombre_archivo:str):
    ...
    with open(nombre_archivo, "r") as archivo: #abrir en modo lectura -- as -- ¿qué nombre le quiero poner al archivo?
        ...
        lista = dict["bzrp"]   #devolver la lista

    return lista


#funcion para crear un json/construirloa  partir de una lista, la  lista
def generar_json(nombre_archivo:str, lista:list):
    dict_json = {}   #empaquetar en un diccionario
    dict_json["bzrp"] = lista
    with open(nombre_archivo, "w") as archivo: #escribir, construir el archivo
        json.dump(dict_json, archivo, indent=4) #dump método para crear   

#--------------------------------------------------------
#leer csv pasar de csv a una lista de diccionarios "abrir", accedo a la clave bzrp, cada campo se separa con coma en el csv
def parse_csv(nombre_archivo:str):
    lista = []
    with open(nombre_archivo, "r") as archivo:
        for linea in archivo:
            print(linea)
            lista_campos = linea.split(",")  #genero una lista 
            video = {}
            video["title"] = lista_campos[0]   #le pongo a cada índice de listas_de_campos a las claves del diccionario
...
def generar_csv(nombre_archivo:str, lista:list): 
    with open(nombre_archivo, "w") as archivo: #abrir en modo escritura
        for video in lista:                    #iterar cada clave
            mensaje = "{0},{1},{2},{3},{4},{5}\n"
